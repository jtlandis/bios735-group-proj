...
all_items_dev |>
  ...
  ggplot(aes(x = model, y = value)) +
    ...
    labs(title = "Deviance Comparison",
         x = "Model",
         y = "Deviance",
         caption = "Hierarchical model is not always better than the single model")
```
...
A  big issue with our data is that promotion is not a good predictor of sales. Some days promotion has no effect on sales while others days are extremely effective.

We can attempt to model some latent variable of promotion effectiveness as a bornoulli distribution with unknown $\pi_i$.
...
gg_panel_a <- panel_a |> ggplot(aes(Date)) +
   ...
   geom_text(aes(label = label, x = x, y = y),
     data = ~summarise(group_by(.x, brand),
