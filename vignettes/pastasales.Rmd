...
theme_minimal() +
labs(title = "Investment by Brand Item", caption = "Each data point represents a brand item. Invesetment is defined as the percent of days promoted over the course of a time series.")
...
It is not necessarily true that the hierarchical model is always better than the single model. The deviance between the two are very similar.

A  big issue with our data is that promotion is not a good predictor of sales. This may be seen as a red heirring as some promotions fail to be effeective on sales, while others days are extremely effective.

We can attempt to model some latent variable of promotion effectiveness as some bournoli distribution.

```{r, em}
#model_all <- data_set_tidy |>
#  group_by(brand, item) |>
#  summarise(
#    model = par_model_mat(pick(DATE, QTY, PROMO),
#      QTY ~ PROMO,
#      nlag = 4
#    ) |> list(),
#    em = {print(cur_group());par_em_effective(model[[1]], col = PROMO,
#          subset = PROMO == 1, tol = 1e-6, diagnostic = TRUE) |> list()}
#  ) |>
#  rowwise() |>
#  mutate(
#    pi = em$pi,
...
#  )
#
#model_all <- mutate(model_all,
#  item = as.integer(item),
#  deviance = get_deviance(em$original_spec),
#  deviance_em = get_deviance(em$spec),
#  loglik_em = ll,
#) |> arrange(brand, item)
#
#plot_data <- all_items_dev |>
#  left_join(model_large_dev, by = c("brand", "item"),
#    suffix = c("_single", "_hierarchical")
#  ) |>
#  left_join(
#    select(model_all, brand, item, deviance_em, loglik_em, pi)
#  ) |>
#  pivot_longer(
#    cols = -c(brand, item, pi),
#    names_to = c("metric","model"),
#    names_pattern = "(.*)_(.*)",
...
#    value = abs(value)
#  ) |>
#  ggplot(aes(x = model, y = value,  color = pi)) +
#    geom_point() +
#    geom_line(aes(group = interaction(brand, item))) +
#    facet_grid(vars(metric), vars(brand), scales = "free_y") +
#    theme_bw() +
#    scale_x_discrete(limits = c("hierarchical", "single", "em")) +
#    scale_color_viridis_c() +
#    labs(title = "Model Comparison - EM")
#
#ggsave("../EM-loglik_deviance.png", .gg)
#.gg
#
#plot_data |>
#  ggplot(aes(x = model, y = loglik,  color = pi)) +
#    geom_point() +
#    geom_line(aes(group = interaction(brand, item))) +
#    facet_wrap(~brand) +
#    theme_minimal() +
