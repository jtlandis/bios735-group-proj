...
##-------------------------------------------##
# Arma model ####
model_fit_TS = function(dat, item, brand, p, q) {
  fit = Arima(
    dat[dat$item == item & dat$brand == brand, ]$QTY,
    order = c(p, 0, q),
...
# Data load ####
train = readRDS(
  system.file("extdata/data_set_tidy_train.rds", package = "pastasales")
)
test = readRDS(
  system.file("extdata/data_set_tidy_test.rds", package = "pastasales")
)
##-------------------------------------------##
...
for (i in seq_len(H)) {
  ...
  y.hist = ts(
    c(as.numeric(train.ts), as.numeric(test.ts[1:i])),
  ...
  fith = Arima(
    y.hist,
    model = fith,
    xreg = c(train.sub$PROMO, test.sub$PROMO[1:i])
  )
}
